...
Banco: `ops bench sidecar` copiado fuera del árbol de Cauce con el layout de gouduet —instancia y producto en repositorios hermanos—, `automation install` del runner y los guards invocados por los shims instalados, como los invoca el runner. Control: el mismo input con el motor 0.98.0 de gouduet-ops, cambiando sólo el enlace del banco. **Corrige al propio caso.** En sesiones reales de Claude Code 2.1.281, el aviso de una tarea de fondo entra por `UserPromptSubmit` con `<task-notification>` y un `prompt_id` nuevo —comprobado—, y **siempre** trae una nota fija: «A task-notification fires each time this agent stops with **no** live background children…». Con ese «no», 0.98.0 leía todo aviso como una negativa, así que la dirección peligrosa de este caso —aprobar lo prohibido— **no ocurre hoy** con avisos reales: el control («No toques api/.env» → aviso → `.env` frenado → «seguí con lo tuyo») dio `pending: []` y reintento frenado con los dos motores. Ocurre con un aviso sin esa nota, que es lo que mide la reproducción de arriba. La que **sí ocurre siempre** con 0.98.0 es la otra mitad: con un pedido sin negación, lo frenado en el turno del aviso no queda pendiente y el «dale» real no aprueba nada (`pending: []`, reintento `exit=2`); con la rama queda pendiente, el «dale» lo aprueba y el reintento pasa. El arreglo corrige las dos sin depender del texto de la nota.
