...
La revisión encontró que lanzar un recorrido aprobaba lo pendiente: `record()` llenaba `approved` con cualquier mensaje humano que no negara, también `/autobuild`, y desde este caso los subagentes del recorrido lo recibían por `confirmed`. Reproducido (el subagente leyó el `.env` pendiente tras `/autobuild`) y corregido en `1527dba8`: un mensaje que lanza un recorrido no contesta un bloqueo. Prueba nueva vista en rojo.
