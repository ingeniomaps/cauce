...
**Cancela igual.** El 2026-09-08 se mergearon los veinte PR de la tanda semanal y quince corridas de
`main` quedaron en `cancelled`.
...
El paso siguiente es barato: un workflow de prueba con el mismo `concurrency` y dos pushes seguidos a una
rama que se llame `main` en un repositorio de descarte, mirando si cancela.
...
- **Arreglarlo cuesta corridas.** Hoy una tanda de veinte merges consume dos corridas completas de la
  suite; con la cancelación desactivada consumiría cuarenta, y treinta y ocho de ellas no pueden
  encontrar nada que la corrida del PR no haya encontrado ya. Verificado: `sourceFiles()`
