...
Banco: `ops bench sidecar` copiado fuera del árbol de Cauce con el layout de gouduet —instancia y producto en repositorios hermanos—, `automation install` del runner y los guards invocados por los shims instalados, como los invoca el runner. Control: el mismo input con el motor 0.98.0 de gouduet-ops, cambiando sólo el enlace del banco. El puente instalado: el campo renombrado (`Command`) y `pre-files` sin archivo reciben `deny` nombrando lo que llegó; `git status` y la entrada vacía pasan. Con el puente de 0.98.0 el campo renombrado pasa con `allow`. **Premisa comprobada con `agy` 1.1.16 real**, capturando la entrada cruda: `pre-shell` llega como `toolCall.name: "run_command"` con `toolCall.args.CommandLine` y `Cwd`, y `pre-files` como `write_to_file` con `TargetFile` y `CodeContent` —los nombres que lee `normalize()`—, con `workspacePaths: []`. Un campo renombrado no se puede provocar con `agy` real; el control es el del puente instalado.

### Revisión del conjunto antes del PR (2026-09-24)

La revisión encontró que el campo de **contenido** tenía el mismo problema que este caso cerró para el comando y el archivo: con `CodeContent` renombrado, `pre-files` traía el archivo y los guards juzgaban un texto vacío —en el entorno de la prueba, `allow` sobre un `DROP TABLE`—. Corregido en `6ec7b9ae`: una llamada de archivos tiene que traer `CodeContent`, `ReplacementContent` o `ReplacementChunks`. Comprobado con `agy` 1.1.16 real que no niega lo legítimo: su edición llega como `replace_file_content` con `ReplacementContent`, y su escritura como `write_to_file` con `CodeContent`.
